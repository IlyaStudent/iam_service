package com.social_media.iam_service.repository.criteria;
    ...
    public Predicate toPredicate(
        ...
        if (Objects.nonNull(request.getKeyword())) {
            Predicate keywordPredicate = criteriaBuilder.or(
                    criteriaBuilder.like(root.get(Post.TITLE_FIELD), "%" + request.getKeyword() + "%"),
                    criteriaBuilder.like(root.get(Post.CONTENT_FIELD), "%" + request.getKeyword() + "%")
            );
    ...
    private void sort(Root<Post> root, CriteriaBuilder criteriaBuilder, CriteriaQuery<?> query) {
        if (Objects.nonNull(request.getSortField())) {
            switch (request.getSortField()) {
                case TITLE -> query.orderBy(criteriaBuilder.desc(root.get(Post.TITLE_FIELD)));
                case CONTENT -> query.orderBy(criteriaBuilder.desc(root.get(Post.CONTENT_FIELD)));
                case LIKES -> query.orderBy(criteriaBuilder.desc(root.get(Post.LIKES_FIELD)));
